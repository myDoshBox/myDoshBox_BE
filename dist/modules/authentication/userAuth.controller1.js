...
const verifyUserEmail = async (req, res) => {
    try {
        ...
        const checkIfBlacklistedToken = await blacklistedToken_model_1.BlacklistedToken.findOne({
            token,
        });
...
const UserLogin = async (req, res) => {
    ...
    try {
        const individualUserToLogin = await individualUserAuth_model1_1.default.findOne({
            email,
        }).select("+password");
        const organizationUserToLogin = await organizationAuth_model_1.default.findOne({
        ...
        if (individualUserToLogin) {
            ...
            if (!individualUserToLogin.email_verified) {
                ...
                await (0, email_utils_1.sendVerificationEmail)(email, verificationToken);
            ...
            const passwordMatch = await individualUserToLogin.comparePassword(user_password);
            if (!passwordMatch) {
            ...
            const { status, message, user: userWithoutPasswordForSession, accessToken, refreshToken, } = await (0, createSessionAndSendToken_util_1.createSessionAndSendTokens)({
            ...
            if (organizationUserToLogin) {
                ...
                if (!organizationUserToLogin.email_verified) {
                    ...
                    await (0, email_utils_1.sendVerificationEmail)(organizationUserToLogin.organization_email, verificationToken);
                    return res.status(200).json({
                        status: "true",
                ...
                const passwordMatch = await organizationUserToLogin.comparePassword(user_password);
                if (!passwordMatch) {
...
//   }
// };
exports.OrganizationUserForgotPassword = (0, catchAsync_1.default)(async (req, res, next) => {
...
exports.organizationUserUpdatePassword = (0, catchAsync_1.default)(async (req, res, next) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
const checkIfUserExist = async (email) => {
    const individualUser = await individualUserAuth_model1_1.default.findOne({
        email,
    });
    if (individualUser)
        return individualUser;
    const organizationUser = await organizationAuth_model_1.default.findOne({
        organization_email: email,
    });
    if (organizationUser)
