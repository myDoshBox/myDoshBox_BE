...
const individualUserRegistration = async (req, res) => {
    try {
...
const individualUserRegistrationGoogle = async (req, res, next) => {
...
const resetIndividualPassword = async (req, res) => {
    ...
    try {
        ...
        const user = await individualUserAuth_model1_1.default.findOne({ email });
        if (!user) {
        ...
        await individualAuthPasswordToken_1.default.findOneAndDelete({ owner: userId });
 ...
 */
const getGoogleUserDetail = async (req, res, next) => {
    try {
        const { code } = req.body;
        if (!code) {
        ...
        const userDetails = await (0, exports.getUserDetails)(tokens.access_token);
        ...
        if (!googleUserExist) {
            const newUser = await individualUserAuth_model1_1.default.create({
                name,
                email,
                email_verified,
                picture,
            ...
            const { status, message, user, accessToken, refreshToken } = await (0, createSessionAndSendToken_util_1.createSessionAndSendTokens)(createSessionAndSendTokensOptions);
            return res.status(201).json({
                status,
                message,
                user,
