...
const paymentForEscrowProductTransaction = async (data) => {
    ...
    const response = await axios_1.default.post(`${API_URL}/transaction/initialize`, {
...
const verifyPaymentForEscrowProductTransaction = async (reference) => {
    const API_URL = process.env.PAYSTACK_BASE_URL;
    const API_KEY = process.env.PAYSTACK_PAYMENT_KEY;
    const response = await axios_1.default.get(`${API_URL}/transaction/verify/${reference}`, {
        headers: {
            Authorization: `Bearer ${API_KEY}`,
            "Content-Type": "application/json",
            "cache-control": "no-cache",
