...
const initiateEscrowProductTransaction = async (req, res, next) => {
    const { 
    // transaction_id,
    ...
    try {
        ...
        // const user = req.user;
        const user = await individualUserAuth_model1_1.default.findOne({ email: buyer_email }); //
        ...
        if (!user) {
            ...
            // buyer pays for the escrow
            const transaction_id = await (0, uuid_1.v4)(); // Generate a random UUID
            // console.log(transaction_id);
            const data = {
                reference: transaction_id,
                amount: transaction_total,
            ...
            // console.log("trans_id_b4_pay", data?.reference);
            const buyerPaysForEscrow = await (0, productsTransaction_paystack_1.paymentForEscrowProductTransaction)(data);
            // console.log(buyerPaysForEscrow);
            // details are saved in the db
...
const verifyEscrowProductTransactionPayment = async (req, res, next) => {
    try {
        const { reference } = req.body;
        console.log("reference", reference);
        const transaction = await productsTransaction_model_1.default.findOne({
        ...
        if (!transaction) {
            ...
            await (0, productsTransaction_paystack_1.verifyPaymentForEscrowProductTransaction)(reference);
            ...
            // });
            const newTerr = await productsTransaction_model_1.default.findOneAndUpdate({ _id: transaction === null || transaction === void 0 ? void 0 : transaction._id }, 
            // {
            //   transaction_id: reference,
            // }
            {
            ...
            await (0, productTransaction_mail_1.sendEscrowInitiationEmailToInitiator)(buyer_email, transaction_id, transaction_total);
            // await sendEscrowInitiationEmail(user?.email, transaction_id);
            // Send email to the vendor
            await (0, productTransaction_mail_1.sendEscrowInitiationEmailToVendor)(transaction_id, vendor_name, vendor_email, product_name, product_price);
            // send response
            res.json({
                status: "success",
...
const getSingleEscrowProductTransaction = async (req, res, next) => {
    try {
...
const getAllEscrowProductTransactionByUser = async (req, res, next) => {
    try {
        // const user = res?.locals?.user;
        ...
        const transactions = await productsTransaction_model_1.default.find({
            $or: [
...
const sellerConfirmsEscrowProductTransaction = async (req, res, next) => {
    ...
    try {
        ...
        const transactionWithConfirmationStatus = await productsTransaction_model_1.default.findOne({
            transaction_id: transaction_id,
        ...
        if (transactionId !== transaction_id) {
            ...
            const checkIfUserExists = await individualUserAuth_model1_1.default.findOne({
                email: vendor_email,
            });
            // const user = res.locals.user;
...
const sellerFillOutShippingDetails = async (req, res, next) => {
    // when they accept/confirm, a message/popup to tell the seller the next steps.
    // mail is sent to the buyer that the seller has agreed and will be sending the goods
    // the mail contains a link to the page where the buyer can click on so that they are redirected to where they can confirm that they like the product and close the escrow.
    ...
    try {
        ...
        const getTransaction = await productsTransaction_model_1.default.findOne({
            transaction_id: transaction_id,
            seller_confirm_status: false,
        });
        ...
        // }
        const user = await individualUserAuth_model1_1.default.findOne({
            email: vendor_email,
        });
        if (transactionId !== transaction_id && sellerConfirmStatus !== false) {
            ...
            const updatedConfirmationStatus = await productsTransaction_model_1.default.findOneAndUpdate({ _id: getTransaction === null || getTransaction === void 0 ? void 0 : getTransaction._id }, {
            ...
            await (0, productTransaction_mail_1.sendShippingDetailsEmailToInitiator)(buyer_email, shipping_company, delivery_person_name, delivery_person_number, delivery_date, pick_up_address);
            // send mail to seller about waiting for delivery to get to buyer
            await (0, productTransaction_mail_1.sendShippingDetailsEmailToVendor)(transactionId, vendor_name, vendor_email, product_name);
...
const getAllShippingDetailsForBuyer = async (req, res, next) => {
    try {
        // const user = res?.locals?.user;
        ...
        const transactions = await shippingDetails_model_1.default.aggregate([
            {
                $lookup: {
...
const getAllShippingDetailsForVendor = async (req, res, next) => {
    try {
        // const user = res?.locals?.user;
        // const { buyer_email, vendor_email } = req.params;
...
//   }
// };
const getAllShippingDetails = async (req, res, next) => {
    try {
        // const user = res?.locals?.user;
        // const { buyer_email, vendor_email } = req.params;
        const { user_email } = req.params;
...
//   }
// };
const buyerConfirmsProduct = async (req, res, next) => {
    const { transaction_id } = req.body;
    if (!transaction_id) {
        return next((0, errorHandling_middleware_1.errorHandler)(400, "Transaction ID is required"));
    }
    try {
        ...
        const fetchShippingDetails = await shippingDetails_model_1.default.aggregate([
            {
                $lookup: {
        ...
        const updateProductTransactionStatus = await productsTransaction_model_1.default.findByIdAndUpdate(product_id, { transaction_status: "completed" }, { new: true });
        if (!updateProductTransactionStatus) {
            return next((0, errorHandling_middleware_1.errorHandler)(500, "Failed to update product status"));
        }
        // Send success emails
        await (0, productTransaction_mail_1.sendSuccessfulEscrowEmailToInitiator)(transaction_id, vendor_name, buyer_email, product_name);
        await (0, productTransaction_mail_1.sendSuccessfulEscrowEmailToVendor)(transaction_id, vendor_name, vendor_email, product_name);
        res.json({
            status: "success",
            message: "Escrow has been completed successfully.",
        });
