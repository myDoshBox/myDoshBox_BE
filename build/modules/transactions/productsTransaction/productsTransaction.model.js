...
// VENDOR BANK DETAILS SCHEMA
const vendorBankDetailsSchema = new mongoose_1.Schema({
    account_name: {
        type: String,
        required: true,
        trim: true,
...
productTransactionSchema.index({ vendor_email: 1 });
productTransactionSchema.index({ transaction_status: 1, payment_released: 1 });
productTransactionSchema.index({ transfer_initiated: 1 }); // ✅ ADD INDEX
// VALIDATION MIDDLEWARE
productTransactionSchema.pre("save", function (next) {
    const transaction = this;
    if (transaction.payment_released &&
        !transaction.vendor_bank_details &&
        transaction.transaction_status === "completed") {
        return next(new Error("Vendor bank details required before releasing payment"));
    }
    if (this.isModified("payment_released") && transaction.payment_released) {
        transaction.payment_released_at = new Date();
    }
    // ✅ ADD MIDDLEWARE FOR transfer_initiated
    if (this.isModified("transfer_initiated") && transaction.transfer_initiated) {
        transaction.transfer_initiated_at = new Date();
    }
    if (this.isModified("transfer_status") &&
        transaction.transfer_status === "success") {
        transaction.transfer_completed_at = new Date();
    }
...
productTransactionSchema.methods.isEligibleForPaymentRelease =
    function () {
        const transaction = this;
        return (transaction.transaction_status === "completed" &&
            !transaction.payment_released &&
            !!transaction.vendor_bank_details &&
            transaction.buyer_confirm_status === true &&
            transaction.verified_payment_status === true);
    };
productTransactionSchema.methods.canMarkAsCompleted = function () {
    const transaction = this;
    return (transaction.transaction_status === "in_transit" &&
        transaction.buyer_confirm_status === true &&
        transaction.shipping_submitted === true);
};
productTransactionSchema.methods.hasTransferFailed = function () {
    const transaction = this;
    return transaction.transfer_status === "failed";
};
productTransactionSchema.methods.needsManualReview = function () {
    const transaction = this;
    return (transaction.requires_manual_review === true ||
        (transaction.transfer_status === "failed" &&
            !transaction.transfer_failure_reason) ||
        (transaction.payment_released && !transaction.transfer_status));
};
