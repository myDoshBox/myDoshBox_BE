...
function generateAccessAndRefreshToken(userObject, sessionId, role) {
    const accessToken = (0, signAndVerifyToken_util_1.signJwt)({ userData: userObject, session: sessionId, role }, { expiresIn: process.env.ACCESS_TOKEN_TTL || ACCESS_TOKEN_EXPIRY });
    const refreshToken = (0, signAndVerifyToken_util_1.signJwt)({ userData: userObject, session: sessionId, role }, { expiresIn: process.env.REFRESH_TOKEN_TTL || REFRESH_TOKEN_EXPIRY });
...
function reIssueAccessToken(_a) {
    return __awaiter(this, arguments, void 0, function* ({ refreshToken, }) {
        ...
        const accessToken = (0, signAndVerifyToken_util_1.signJwt)({ userData: user, session: session._id, role: session.role }, { expiresIn: accessTokenTTL });
        return accessToken;
    });
}
